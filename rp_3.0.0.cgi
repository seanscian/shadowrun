...
$emote_name = /^(?:(?:La|El|The)? )?\w+/.match(sl_user)[0]
#	STDERR.puts(sl_user)	#
#	STDERR.puts($emote_name)	#
...
def mention(message,highlight)
	$rpdb.execute("SELECT DISTINCT substr(trim(charname),1,instr(trim(charname)||' ',' ')-1) FROM characters WHERE config IS #{$db_config} AND slack_user IS NOT \"#{$user_id}\" AND GM IS null").each {
		...
		if /\b#{x[0]}\b/.match(message.to_s)
			$rpdb.execute("SELECT DISTINCT slack_user FROM characters WHERE charname LIKE \"#{x[0]}%\" AND config IS #{$db_config}").each {
				|n|
					# The channel notation seems to change in Slack. It used to be <#123456|name>, then just #123456, now it’s <#123456>
				notify_message = {
	...
	# These were formerly external bash scripts. Moving them internal
	# to the code means I can close STDOUT to ensure Slack gets the fast
	# response it requires.

	# I was initially troubled by putting something so game-specific in-
	# line in the code, but the Star Wars roller tables and Shadowrun 5e
	# rollng format is pretty darn specific and I’m OK with it, so this
